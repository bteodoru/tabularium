...
def test_fine_sand_medium():
    r = get_deformation_modulus(Soil.FINE_SAND, RelativeDensity.MEDIUM)
    assert r.valid is True
    assert r.e_modulus == pytest.approx(25_000.0)


def test_fine_sand_dense():
    r = get_deformation_modulus(Soil.FINE_SAND, RelativeDensity.DENSE)
    assert r.valid is True
    assert r.e_modulus == pytest.approx(35_000.0)


def test_silty_sand_medium():
    r = get_deformation_modulus(Soil.SILTY_SAND, RelativeDensity.MEDIUM)
    assert r.valid is True
    assert r.e_modulus == pytest.approx(18_000.0)


def test_silty_sand_dense():
    r = get_deformation_modulus(Soil.SILTY_SAND, RelativeDensity.DENSE)
    assert r.valid is True
    assert r.e_modulus == pytest.approx(30_000.0)
...
def test_invalid_relative_density():
    r = get_deformation_modulus(Soil.FINE_SAND, "invalid_density")
    assert r.valid is False
    assert len(r.errors) == 1
